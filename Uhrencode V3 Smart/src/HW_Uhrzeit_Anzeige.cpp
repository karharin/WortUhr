...
void HW_Anzeige::setTime(int hour, int minute)
{
    ...
    if (minute == 0)
    {
        ...
        if (hour == 1) {
           this->setColorLetter(hours[hour-1][1], pixels->Color(0, 0, 0));
        }
    }
    else if (minute == 15) 
    {
    ...
    if (AnzeigeControlStruct.partyModeActive)
    {
        ...
        for (int i = 0; i < NUM_COLS; i++)
        {
            for (int j = 0; j < NUM_ROWS; j++)
            {
                ...
                this->setColorLetter(XY(i, j), pixels->ColorHSV(((index + ms / 200) % 255) * 257, 255, AnzeigeControlStruct.currentBrightness));
            }
        }
        pixels->show();
    }
...
{
    // get brighness from light sensor
    float brightnessReading = lightSensor->readLightLevel();
    // Serial.println("Brightness reading: " + String(brightnessReading));

    int brightnessRangeUser = AnzeigeControlStruct.maxBrightness - AnzeigeControlStruct.minBrightness;
    float m = (float)brightnessRangeUser / (float)BRIGHTNESS_RANGE;
    float sens = (float)AnzeigeControlStruct.sensitivity / 10.0;
    int newBrightness = AnzeigeControlStruct.minBrightness + (int)(brightnessReading * m * sens);

    if (newBrightness > AnzeigeControlStruct.maxBrightness)
    {
        newBrightness = AnzeigeControlStruct.maxBrightness;
    }
    else if (newBrightness < AnzeigeControlStruct.minBrightness)
    {
        newBrightness = AnzeigeControlStruct.minBrightness;
    }
    // Serial.println("New brightness: " + String(newBrightness));
